...
function App(){
 ...
 useEffect(()=>{
  ...
  const loadUser=async(id:string,email:string)=>{
   const {data:p,error}=await supabase.from("profiles").select("full_name,role,department").eq("id",id).single();
   if(!active)return;
   if(error){setNotice("Your account exists, but its portal profile could not be loaded.");setUser(null);return;}
   setUser({id,email,name:p.full_name,role:p.role as Role,department:p.department||""});
  };
 ...
 async function saveStudentProfile(values:{fullName:string;department:string}){
  if(!user)return false;
  setNotice("");
  if(configured){
   const {error}=await supabase.from("profiles").update({full_name:values.fullName,department:values.department||null}).eq("id",user.id);
   if(error){setNotice(error.message);return false;}
  }
  setUser({...user,name:values.fullName,department:values.department});
 ...
 const studentNotifications:NotificationAppointment[]=booked.filter((item)=>Boolean(item.status)).map((item)=>({id:item.appointment_id||item.id,status:item.status!,starts_at:item.starts_at,faculty_name:item.faculty_name,student_name:user.name,topic:item.topic||item.expertise,location:item.location}));
 ...
 {view==="home"&&<Dashboard user={user} booked={booked} go={nav}/>} {view==="find"&&<FindFaculty query={query} setQuery={setQuery} slots={filtered} select={setSelected}/>} {view==="schedule"&&<Schedule booked={booked} cancel={cancelRequest} reschedule={beginReschedule} busy={submitting}/>} {view==="assistant"&&<Chat chat={chat} question={question} setQuestion={setQuestion} ask={ask}/>} {view==="profile"&&<StudentProfile user={user} save={saveStudentProfile}/>}</main>
...
function StudentProfile({user,save}:{user:User;save:(values:{fullName:string;department:string})=>Promise<boolean>}){
 ...
 const submit=async(e:FormEvent<HTMLFormElement>)=>{
  ...
  setSaving(true);
  const saved=await save({fullName:String(form.get("full_name")||"").trim(),department:String(form.get("department")||"").trim()});
  setSaving(false);
 ...
 return <>
  ...
  <section className="student-profile-layout">
   ...
   <article className="student-details-card"><button className="edit-profile-button" onClick={()=>setEditing(true)}>Edit profile</button><Info l="Full name" v={user.name}/><Info l="Email" v={user.email}/><Info l="Course and year" v={user.department||"Not provided"}/><Info l="Account type" v="Student"/><Info l="Profile status" v="Active FacultyConnect account"/></article>
  </section>
...
function AdminPages({view,user}:{view:AView;user:User}){
 ...
 if(view==="ahome")return <>{feedback}<Head label="MISO ADMINISTRATION" title="Pilot Overview" copy="Monitor the CLIRDEC pilot before university-wide expansion." action="Export summary"/><Stats data={[[String(data.users.length),"Registered users"],[String(data.appointments.length),"Consultations"],[String(pending),"Pending requests"],["0","Double bookings"]]}/><div className="workspace-grid"><Work title="Today's appointments">{data.appointments.filter(item=>item.status==="confirmed").slice(0,6).map(item=><Line key={item.id} a={new Date(item.starts_at).toLocaleString([], {month:"short",day:"numeric",hour:"numeric",minute:"2-digit"})} b={item.topic} c={item.student_name}/>)}{!confirmed&&<div className="empty-card">No confirmed consultations.</div>}</Work><Work title="Recent activity"><Line a="Now" b="Pilot data refreshed" c="Live Supabase records"/><Line a={String(data.faqs.filter(item=>item.status==="approved").length)} b="Approved FAQ entries" c="Available to students"/><Line a={String(completed)} b="Completed consultations" c="Pilot records"/></Work></div></>;
