...
function App(){
 ...
 return <div className="app"><header className="topbar"><button className="brand-button" onClick={()=>nav("home")}><BrandLogo/><span><b>CLSU FacultyConnect</b><small>Managed by MISO · CLIRDEC pilot</small></span></button><div className="top-actions"><button className="icon-button" aria-label="Notifications"><span className="bell-icon">♧</span><span className="dot"/></button><button className="profile-chip" onClick={()=>nav("profile")} aria-label="Open my profile"><span>{user.name.split(" ").map(part=>part[0]).join("").slice(0,2)}</span><i><b>{user.name}</b><small>Student</small></i></button><button className="menu-button" onClick={()=>setMenu(!menu)} aria-label="Toggle menu">☰</button></div></header>
