...
function Chat({
  ...
  const submit = async (event: FormEvent) => {
    if (!PRODUCTION_SECURITY_READY) {
      event.preventDefault();
      setChatActionStatus(
        "The chatbot security service is temporarily unavailable.",
      );
      return;
    }
    ...
    if (trustLoading) {
      event.preventDefault();
      setChatActionStatus(
        "Preparing the protected chat session. Please wait.",
      );
      return;
    }

    if (TURNSTILE_SITE_KEY && !chatTrusted && !captchaToken) {
      event.preventDefault();
      setChatActionStatus(
        "Please complete the Verify you are human check before sending.",
      );
      return;
    }
