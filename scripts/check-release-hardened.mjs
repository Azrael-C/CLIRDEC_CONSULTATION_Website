...
const chatbotDockerfile = read("chatbot/Dockerfile");
const chatbotRequirements = read("chatbot/requirements.txt");
const resendWebhook = read("supabase/functions/resend-webhook/index.ts");
const deliveryMigration = read("supabase/migrations/20260814110000_email_delivery_events.sql");
const deliveryEventTypeMigration = read("supabase/migrations/20260820170848_track_resend_suppressed_events.sql");
...
const reportLengthMigration = read("supabase/migrations/20260926090000_expand_user_report_message.sql");
...
assertIncludes(chatbot, "MAX_REQUEST_BODY_BYTES", "Chatbot request body limit");
assertIncludes(chatbot, "REQUEST_TIMEOUT_SECONDS", "Chatbot processing timeout");
assertIncludes(chatbot, 'response.headers["X-Robots-Tag"]', "Chatbot API indexing protection");
assertIncludes(chatbotDockerfile, "USER facultyconnect", "Non-root chatbot container");
assertIncludes(chatbotDockerfile, "--limit-concurrency 100", "Chatbot container concurrency limit");
assertIncludes(chatbotDockerfile, "--limit-max-requests 1000", "Chatbot worker recycling");
assertIncludes(chatbotRequirements, "click==8.4.2", "Pinned chatbot dependency");
assertIncludes(app, "PRODUCTION_SECURITY_READY", "Production authentication fail-closed protection");
assertIncludes(app, "Clear chat", "Student chatbot reset control");
assertIncludes(app, "Include as much context as needed", "Variable-length issue reports");
assertIncludes(vercel, "Cross-Origin-Opener-Policy", "Cross-origin opener isolation header");
assertIncludes(vercel, "script-src-attr 'none'", "Inline script attribute protection");
assertIncludes(vercel, "upgrade-insecure-requests", "Mixed-content upgrade policy");
assertIncludes(resendWebhook, "new Webhook(webhookSecret).verify", "Resend webhook signature verification");
assertIncludes(resendWebhook, "readTextLimited", "Resend webhook streaming body limit");
...
assertIncludes(reportLengthMigration, "between 1 and 5000", "Variable-length report storage bound");
assertIncludes(reportLengthMigration, "Too many reports", "User report rate limit");
