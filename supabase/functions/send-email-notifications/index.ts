...
Deno.serve(async (request) => {
  ...
  const portalUrl =
    Deno.env.get("PORTAL_URL") || "https://clsu-faculty-connect.vercel.app";
  const from =
    Deno.env.get("EMAIL_FROM") || "CLSU FacultyConnect <onboarding@resend.dev>";
  ...
  // One scheduled invocation creates due reminders and drains the queue.
  ...
  const recipientIds = [
    ...new Set(
      claimed.map((item: { recipient_id: string }) => item.recipient_id),
    ),
  ];
  ...
  if (profileError) {
    await supabase
      ...
      .in(
        "id",
        claimed.map((item: { id: string }) => item.id),
      );
  ...
  for (const item of claimed) {
    ...
    try {
      if (!profile?.email)
        throw new Error("Recipient profile or email is missing");
      ...
      const retryAt = new Date(
        Date.now() + 5 * 60_000 * 2 ** Math.max(0, item.attempts - 1),
      );
      await supabase
        .from("email_notifications")
        .update({
